...
try:
    from ml.data.dwh import DWHManager, create_schema, drop_schema, ingest_house_data, validate_ingestion, get_schema_info
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Setup DuckDB DWH and ingest house data")
    parser.add_argument(
        "--csv-file", 
        type=str, 
    ...
    try:
        ...
        if result['ingestion_status'] == 'success':
            ...
            print(f"  - Rows with missing dimensions: {result['fact_data_stats']['rows_with_missing_dimensions']}")
            ...
            if result['summary_statistics']:
                ...
                if location_analytics:
                    ...
                    for i, location in enumerate(location_analytics[:3]):
                        print(f"  {i+1}. {location['location_name']}: ${location['avg_price']:,.0f} avg price")
