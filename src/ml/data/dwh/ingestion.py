        ...
        if create_schema_if_not_exists:
            if not dwh_manager.table_exists("raw_house_data"):
                ...
                try:
                    ...
                    logger.info(
                        f"Schema created successfully. Available tables: {tables}"
                    )
...
def _load_and_validate_csv(csv_file_path: str) -> pd.DataFrame:
    ...
    try:
        ...
        if final_count < initial_count:
            logger.warning(
                f"Removed {initial_count - final_count} rows with invalid data"
            )
...
def _transform_and_load_fact_data(
    df: pd.DataFrame, dwh_manager: DWHManager
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        for i in range(min(3, len(df_fact))):
            row = df_fact.iloc[i]
            logger.debug(
                f"Row {i}: house_id={row['house_id']}, location_id={row['location_id']}, condition_id={row['condition_id']}, year_built_id={row['year_built_id']}"
            )

        # Remove rows with missing dimension mappings
        initial_count = len(df_fact)
        df_fact = df_fact.dropna(
            subset=["location_id", "condition_id", "year_built_id"]
        )
        ...
        for i, row in df_fact.iterrows():
            ...
            if i < 3:  # 最初の3行のみログ出力
                logger.debug(
                    f"Row {i}: transaction_id={transaction_id}, house_id={house_id}, location_id={location_id}, condition_id={condition_id}, year_built_id={year_built_id}"
                )
        ...
        logger.info(
            f"Prepared {len(data_tuples)} rows for fact_house_transactions insertion"
        )
...
def _get_location_mapping(dwh_manager: DWHManager) -> Dict[str, int]:
    ...
    query = (
        "SELECT location_name, location_id FROM dim_locations WHERE is_active = TRUE"
    )
...
def _get_condition_mapping(dwh_manager: DWHManager) -> Dict[str, int]:
    ...
    query = (
        "SELECT condition_name, condition_id FROM dim_conditions WHERE is_active = TRUE"
    )
    result = dwh_manager.execute_query(query)
...
def validate_ingestion(dwh_manager: Optional[DWHManager] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if dwh_manager.table_exists("raw_house_data") and dwh_manager.table_exists(
            "fact_house_transactions"
        ):
            ...
            if raw_count != fact_count:
                validation_results["errors"].append(
                    f"Data inconsistency: raw_count={raw_count}, fact_count={fact_count}"
                )
