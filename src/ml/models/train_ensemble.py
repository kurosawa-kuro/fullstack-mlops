...
def main(args):
    ...
    with mlflow.start_run(run_name=f"ensemble_{method}_training"):
        logger.info(f"Training {method} ensemble with {len(base_models_cfg)} base models")
        
        # Train ensemble
        ensemble.fit(X_train, y_train)
        
        # Evaluate ensemble
        ensemble_results = evaluate_model(ensemble, X_test, y_test, f"{method.title()} Ensemble")
        ...
        preprocessor_save_path = f"{args.models_dir}/trained/{model_name}_preprocessor.pkl"
        ...
        mae_improvement = ((best_base_mae - ensemble_results['mae']) / best_base_mae) * 100
